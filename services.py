...
async def get_ufc_events() -> list[dict]:
    """Получает предстоящие бои MMA с коэффициентами."""
...
def parse_fights_from_odds(events: list[dict]) -> dict[str, list[dict]]:
    """
    Группирует бои по дате ивента.
    Все бои одного дня идут под одним ивентом.
    """
    ...
    for event in events:
        ...
        try:
            ...
            event_name = f"UFC / MMA — {date_str}"
        except Exception:
            event_name = "UFC / MMA Event"

        # Берём лучшие коэффициенты из всех букмекеров
                ...
                if market.get("key") == "h2h":
                    ...
                    for outcome in outcomes:
                        if outcome["name"] == home:
                            cur = odds_h2h["home"]
                            if cur is None or outcome["price"] > cur:
